#!/usr/bin/env python3
"""
Non-Stationary Pose-Aware Planner - Gibbs Kernel + Position Uncertainty

Combines the Gibbs non-stationary kernel with Monte Carlo expected
information gain under position uncertainty.

Key difference from pose_aware_planner.py:
- GP kernel has spatially varying lengthscale l(x)
- l(x) is learned online via marginal likelihood optimization
- Position uncertainty interacts heterogeneously with the non-stationary kernel:
  * Near hotspot boundaries: short l(x), so sigma_x/l(x) is large -> big effect
  * In smooth regions: long l(x), so sigma_x/l(x) is small -> negligible effect
- This causes genuinely different planning decisions from the stationary version

Acquisition function:
    x*_{k+1} = argmax  U_k(x) - lambda * c(x_k, x_{k+1})

Where U_k(x) = E_{x_tilde ~ N(x, Sigma_x)} [Delta(x_tilde)]
             ~= (1/M) sum_{j=1}^M Delta(x + xi_j)

And Delta uses the Gibbs kernel GP posterior variance.
"""
...
class NonstationaryPoseAwareSampler(Node):
    """Non-stationary pose-aware planner with Gibbs kernel + MC expected info gain"""

    MAX_SAMPLES = 100

    def __init__(self):
        super().__init__('nonstationary_pose_aware_sampler')
        ...
        try:
            self.viz = LiveVisualizer(
                title=f'NS-PoseAware - {self.field_type} (MC)',
        ...
        self.get_logger().info(f'Non-Stationary POSE-AWARE Sampler initialized')
        self.get_logger().info(f'  Kernel: Gibbs (spatially varying lengthscale)')
        self.get_logger().info(f'  Acquisition: E[info] under pose noise (MC)')
    ...
    def _create_trial_directory(self):
        ...
        base_dir = workspace_root / 'data' / 'trials' / 'nonstationary_pose_aware' / self.field_type
    ...
    def _save_config(self):
        config = {
            'method': 'nonstationary_pose_aware',
            'description': 'Non-stationary Gibbs kernel + expected info gain under pose noise (MC)',
            'kernel': 'gibbs',
    ...
    def _plot_reconstruction_comparison(self, gp_mean, gp_var, error):
        ...
        plt.suptitle(f'NS-PoseAware - {self.field_type} (Trial {self.trial_num}) - RMSE: {rmse:.3f}°C',
    ...
    def _run_initial_sampling(self):
        ...
        if dist < 0.5 and self.current_temp is not None:
            ...
            if self.waypoint_idx >= len(self.initial_waypoints):
                ...
                self.get_logger().info('Starting adaptive sampling (NS + pose-aware)')
    ...
    def _greedy_single_step(self, current_pos):
        """Expected info gain under pose noise + non-stationary kernel."""
    ...
    def _finish_mission(self):
        ...
        self.get_logger().info('MISSION COMPLETE (NS Pose-Aware)')
...
def main(args=None):
    ...
    node = NonstationaryPoseAwareSampler()
    try:
        rclpy.spin(node)
