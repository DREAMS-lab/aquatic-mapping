#!/usr/bin/env python3
"""
Non-Stationary Hotspot-Guided Pose-Aware Planner - Gibbs Kernel + Kac-Rice + MC

Two-phase adaptive sampling with pose uncertainty awareness:
  Phase 1 (Exploration, samples 1-40): Expected info gain E[Δ(x̃)] under pose noise.
  Phase 2 (Exploitation, samples 41-100): Kac-Rice hotspot detection + multiplicative
    hotspot-weighted expected info gain.

Acquisition function:
  Phase 1:  x* = argmax  U(x) - λ·c(x)         where U(x) = E[Δ(x̃)]
  Phase 2:  x* = argmax  U_w(x) - λ·c(x)       where U_w(x) = E[Δ(x̃)·w(x̃)]

The hotspot weight w(x̃) is applied INSIDE the MC expectation, so it accounts
for where the robot might actually end up, not just the commanded position.
"""
...
def expected_information_gain_vectorized(candidates, gp, noise_var, position_cov, n_mc_samples=30):
    """
    Vectorized expected info gain for ALL candidates at once.

    U(x_i) = E[Delta(x_tilde)] ~= (1/M) sum Delta(x_i + xi_j)

    One GP prediction call for all N*M noisy locations.
    """
...
def expected_hotspot_weighted_information_gain_vectorized(
    candidates, gp, noise_var, position_cov, hotspot_weights, n_mc_samples=30
):
    """
    Hotspot-weighted expected info gain with weight applied INSIDE the MC expectation.

    U_w(x_i) = E[Delta(x_tilde) * w(x_tilde)]
             ~= (1/M) sum_{j=1}^M Delta(x_i + xi_j) * w(x_i + xi_j)

    The weight w(x~) is evaluated at each perturbed location, not the commanded location.
    This accounts for where the robot might actually end up.

    Parameters
    ----------
    candidates : (N, 2) array — commanded candidate locations
    gp : NonstationaryGPModel
    noise_var : float
    position_cov : (2, 2) array — position covariance
    hotspot_weights : dict with keys 'hotspots' (list of dicts with 'lat','lon'),
    ...
    all_info_gains = information_gain(all_variances, noise_var)
    all_info_gains = all_info_gains.cpu().numpy()  # (N*M,)

    # Compute w(x~) at each perturbed location
    all_weights = np.ones(len(all_noisy_locations))
    alpha = hotspot_weights['alpha']
    radius = hotspot_weights['radius']
    for hotspot in hotspot_weights['hotspots']:
        hx, hy = hotspot['lat'], hotspot['lon']
        dists_sq = (all_noisy_locations[:, 0] - hx)**2 + (all_noisy_locations[:, 1] - hy)**2
        all_weights += alpha * np.exp(-dists_sq / (2 * radius**2))

    # Multiply info gain by weight, then average over MC samples
    weighted_gains = (all_info_gains * all_weights).reshape(n_candidates, n_mc_samples)
    expected_weighted_info = np.mean(weighted_gains, axis=1)

    return expected_weighted_info
...
class LiveVisualizer:
    ...
    def __init__(self, title="NS-PoseAware Sampler", output_dir=None):
    ...
    def update(self, gp, candidates, scores, selected_idx, current_pos, target_pos,
        ...
        self.ax_acq.set_title(f'E[Acquisition] (Step {step})')
...
class NonstationaryHotspotPoseAwareSampler(Node):
    """Non-stationary hotspot-guided pose-aware planner with Gibbs kernel + MC expected info gain + Kac-Rice"""

    MAX_SAMPLES = 100

    def __init__(self):
        super().__init__('nonstationary_hotspot_pose_aware_sampler')
        ...
        self.declare_parameter('n_mc_samples', 30)
        ...
        self.uncertainty_scale = self.get_parameter('uncertainty_scale').value
        ...
        try:
            self.viz = LiveVisualizer(
                title=f'NS-Hotspot-PoseAware - {self.field_type} (MC)',
        ...
        self.get_logger().info(f'Non-Stationary HOTSPOT-GUIDED POSE-AWARE Sampler initialized')
        self.get_logger().info(f'  Kernel: Gibbs (spatially varying lengthscale)')
        self.get_logger().info(f'  Acquisition: E[info * w(x~)] under pose noise (MC)')
        ...
        self.get_logger().info(f'  Grid: {self.grid_size}x{self.grid_size}, l=[{self.l_min},{self.l_max}]')
        ...
        self.get_logger().info(f'  Field: {self.field_type}, Trial: {self.trial_num}')
    ...
    def _create_trial_directory(self):
        ...
        base_dir = workspace_root / 'data' / 'trials' / 'nonstationary_hotspot_pose_aware' / self.field_type
    ...
    def _save_config(self):
        config = {
            'method': 'nonstationary_hotspot_pose_aware',
            'description': 'Non-stationary Gibbs kernel + Kac-Rice hotspot-guided + MC expected info gain',
            'kernel': 'gibbs',
    ...
    def _save_ground_truth(self):
        np.savez(self.output_dir / 'ground_truth.npz', X=self.gt_X, Y=self.gt_Y, field=self.gt_field)
    ...
    def _compute_reconstruction_metrics(self):
        ...
        self.get_logger().info(f'Reconstruction: RMSE={rmse:.3f}, MAE={mae:.3f}')
        return metrics
    ...
    def _plot_reconstruction_comparison(self, gp_mean, gp_var, error):
        ...
        axes[1, 0].set_title('GP Variance', fontsize=12, fontweight='bold')
        ...
        if samples_arr is not None and len(samples_arr) >= 3:
            try:
                ...
                coverage = (hull.volume / 625.0) * 100
        ...
        plt.suptitle(f'NS-Hotspot-PoseAware - {self.field_type} (Trial {self.trial_num}) - RMSE: {rmse:.3f}°C',
    ...
    def odometry_callback(self, msg):
        ...
        # Build position covariance matrix for MC expected info gain
    ...
    def _run_initial_sampling(self):
        ...
        if time.time() - self._last_log_time > 5.0:
            self.get_logger().info(f'Initial: wp {self.waypoint_idx+1}/{len(self.initial_waypoints)}, dist={dist:.2f}m')
        ...
        if dist < 0.5 and self.current_temp is not None:
            ...
            if self.waypoint_idx >= len(self.initial_waypoints):
                ...
                self.get_logger().info('Starting adaptive sampling (NS + hotspot + pose-aware)')
    ...
    def _plan_next_sample(self):
        ...
        scores_np = all_scores if isinstance(all_scores, np.ndarray) else all_scores
    ...
    def _run_final_hotspot_analysis(self):
        ...
        try:
            ...
            peaks = detect_and_plot_peaks(
                ...
                kernel_type='gibbs',
                y_mean=0.0,
                gp_label='NS-Hotspot-PoseAware GP',
            )
    ...
    def _greedy_single_step(self, current_pos):
        """Expected info gain under pose noise + optional hotspot weighting inside MC."""
        if self.sampling_phase == 'EXPLOIT' and self.detected_hotspots:
            # Phase 2: U_w(x) = E[Δ(x̃)·w(x̃)] — weight inside MC expectation
            hotspot_params = {
                'hotspots': self.detected_hotspots,
                'alpha': self.hotspot_alpha,
                'radius': self.hotspot_radius,
            }
            expected_info_gains = expected_hotspot_weighted_information_gain_vectorized(
                self.candidates, self.gp, self.noise_var,
                self.position_cov, hotspot_params, self.n_mc_samples
            )
        else:
            # Phase 1: U(x) = E[Δ(x̃)] — standard MC expected info gain
    ...
    def _finish_mission(self):
        ...
        self.get_logger().info('MISSION COMPLETE (NS-Hotspot-PoseAware)')
    ...
    def _plot_hotspot_sampling_analysis(self):
        ...
        try:
            ...
            plt.suptitle(f'NS-Hotspot-PoseAware - {self.field_type} (Trial {self.trial_num})',
...
def main(args=None):
    ...
    node = NonstationaryHotspotPoseAwareSampler()
    try:
        rclpy.spin(node)
