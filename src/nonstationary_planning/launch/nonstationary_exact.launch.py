#!/usr/bin/env python3
"""
Non-Stationary Exact Planner Launch File

Launches everything needed for non-stationary exact planning experiment:
  - Rover infrastructure (TF, rover_monitor, robot_state_publisher)
  - Selected field generator ONLY
  - RViz with field-specific config
  - Non-stationary exact planner node (Gibbs kernel + greedy info gain)

Usage:
  ros2 launch nonstationary_planning nonstationary_exact.launch.py field_type:=radial
...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        # === NON-STATIONARY EXACT PLANNER ===
        TimerAction(
            period=5.0,
            actions=[
                Node(
                    package='nonstationary_planning',
                    executable='nonstationary_exact_planner.py',
                    name='nonstationary_exact_planner',
                    output='screen',
                    emulate_tty=True,
                    parameters=[{
