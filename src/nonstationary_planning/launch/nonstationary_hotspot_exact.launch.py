#!/usr/bin/env python3
"""
Non-Stationary Hotspot-Guided Exact Planner Launch File

Launches everything needed for non-stationary hotspot-guided exact planning:
  - Rover infrastructure (TF, rover_monitor, robot_state_publisher)
  - Selected field generator ONLY
  - RViz with field-specific config
  - Non-stationary hotspot exact planner node (Gibbs kernel + Kac-Rice exploitation)

Usage:
  ros2 launch nonstationary_planning nonstationary_hotspot_exact.launch.py field_type:=radial
...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        # === NON-STATIONARY HOTSPOT-GUIDED EXACT PLANNER ===
        TimerAction(
            period=5.0,
            actions=[
                Node(
                    package='nonstationary_planning',
                    executable='nonstationary_hotspot_exact_planner.py',
                    name='nonstationary_hotspot_exact_planner',
                    output='screen',
                    emulate_tty=True,
                    parameters=[{
