#!/usr/bin/env python3
"""
Non-Stationary Hotspot-Guided Pose-Aware Planner Launch File

Launches everything needed for non-stationary hotspot-guided pose-aware planning:
  - Rover infrastructure (TF, rover_monitor, robot_state_publisher)
  - Selected field generator ONLY
  - RViz with field-specific config
  - Non-stationary hotspot pose-aware planner node (Gibbs kernel + Kac-Rice + MC)

Usage:
  ros2 launch nonstationary_planning nonstationary_hotspot_pose_aware.launch.py field_type:=radial
...
def generate_launch_description():
    ...
    exploration_samples_arg = DeclareLaunchArgument('exploration_samples', default_value='40')
    hotspot_recheck_interval_arg = DeclareLaunchArgument('hotspot_recheck_interval', default_value='10')
    hotspot_alpha_arg = DeclareLaunchArgument('hotspot_alpha', default_value='2.0')
    hotspot_radius_arg = DeclareLaunchArgument('hotspot_radius', default_value='3.0')
    ...
    return LaunchDescription([
        ...
        exploration_samples_arg, hotspot_recheck_interval_arg,
        hotspot_alpha_arg, hotspot_radius_arg,
        ...
        # === NON-STATIONARY HOTSPOT-GUIDED POSE-AWARE PLANNER ===
        TimerAction(
            period=5.0,
            actions=[
                Node(
                    package='nonstationary_planning',
                    executable='nonstationary_hotspot_pose_aware_planner.py',
                    name='nonstationary_hotspot_pose_aware_planner',
                    output='screen',
                    emulate_tty=True,
                    parameters=[{
                        ...
                        'exploration_samples': LaunchConfiguration('exploration_samples'),
                        'hotspot_recheck_interval': LaunchConfiguration('hotspot_recheck_interval'),
                        'hotspot_alpha': LaunchConfiguration('hotspot_alpha'),
                        'hotspot_radius': LaunchConfiguration('hotspot_radius'),
                    }]
                )
            ],
        ),
