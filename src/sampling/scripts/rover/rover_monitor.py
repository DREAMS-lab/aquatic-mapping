...
def quat_ned_to_enu(q_ned):
    """
    ned to enu quaternion conversion
    CHANGE THIS LINE TO TEST DIFFERENT OPTIONS:
    """
    qw, qx, qy, qz = float(q_ned[0]), float(q_ned[1]), float(q_ned[2]), float(q_ned[3])

    # TRY THESE OPTIONS (uncomment one):
    # return np.array([qw, qx, qy, qz], dtype=float)      # 1: identity
    # return np.array([qw, qy, qx, qz], dtype=float)      # 2: swap x,y
    # return np.array([qw, qy, qx, -qz], dtype=float)     # 3: swap x,y, negate z
    return np.array([qw, -qy, -qx, qz], dtype=float)      # 4: negate x,y (CURRENT)
    # return np.array([qw, -qy, -qx, -qz], dtype=float)  # 5: negate all
    # return np.array([qw, -qx, -qy, qz], dtype=float)   # 6: negate x,y no swap
    # return np.array([qw, -qx, -qy, -qz], dtype=float)  # 7: negate x,y,z no swap
    # return np.array([qw, qx, qy, -qz], dtype=float)    # 8: just negate z
...
class RoverMonitor(Node):
    ...
    def odom_callback(self, msg):
        ...
        # Minimal periodic logging (every 200 messages ~ 0.1 Hz / 10 seconds)
        self.msg_count += 1
    ...
    def publish_tf(self, odom_msg, p_enu):
        ...
        # Orientation: Convert PX4 NED quaternion to ROS ENU quaternion
        q_ned = np.array([
            odom_msg.q[0],  # qw
            odom_msg.q[1],  # qx
            odom_msg.q[2],  # qy
