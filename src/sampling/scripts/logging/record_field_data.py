...
class FieldDataRecorder(Node):
    def __init__(self, field_name, trial_number):
        ...
        self.csv_writer.writerow(['x', 'y', 'temperature', 'vx', 'vy',
                                   'cov_xx', 'cov_xy', 'cov_yy', 'vel_var_x', 'vel_var_y'])
    ...
    def odom_callback(self, msg):
        ...
        self.current_odom = msg  # store full message for covariance
    ...
    def record_data_point(self):
        ...
        # Write to CSV: x, y, temp, vx, vy, cov_xx, cov_xy, cov_yy, vel_var_x, vel_var_y
        self.csv_writer.writerow([
            float(self.current_odom.position[0]),  # x
            float(self.current_odom.position[1]),  # y
            float(self.current_temp),              # temperature
            float(self.current_odom.velocity[0]),  # vx
            float(self.current_odom.velocity[1]),  # vy
            float(self.current_odom.position_variance[0]),  # cov_xx (σ_x²)
            0.0,                                            # cov_xy (assume 0)
            float(self.current_odom.position_variance[1]),  # cov_yy (σ_y²)
            float(self.current_odom.velocity_variance[0]),  # vel_var_x
            float(self.current_odom.velocity_variance[1])   # vel_var_y
        ])
        self.csv_file.flush()
...
def main(args=None):
    if len(sys.argv) < 3:
        print("\nUsage: ros2 run sampling record_field_data.py <field_name> <trial_number>\n")
        return
