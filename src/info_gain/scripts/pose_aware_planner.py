...
Step 7 — Receding-horizon planning:
    x*_{k+1:k+H} = argmax  SUM_{h=1}^{H} U_k(x_{k+h}) - λ * SUM_{h=0}^{H-1} c(x_{k+h}, x_{k+h+1})
...
class LiveVisualizer:
    ...
    def __init__(self, title="Pose-Aware Sampler"):
        plt.ion()
...
class PoseAwareSampler(Node):
    ...
    def __init__(self):
        ...
        # Will be updated from PX4 EKF in odometry_callback
        ...
        try:
            self.viz = LiveVisualizer(title=f'Planner-Aware Sampler - {self.field_type} (H={self.horizon}, σ from PX4 EKF)')
    ...
    def _create_trial_directory(self):
        ...
        base_dir = workspace_root / 'src' / 'info_gain' / 'data' / 'trials' / 'pose_aware' / self.field_type
    ...
    def odometry_callback(self, msg):
        ...
        The position covariance matrix Σ_x for pose-aware planning is:
            ...
            Σ_x = diag(σ²_x, σ²_y) = diag(σ²_east, σ²_north)
        ...
        # Position variance: Extract from PX4 EKF
        # NED frame: [σ²_north, σ²_east, σ²_down]
        # For 2D planning, we use ENU convention: [σ²_east, σ²_north] = [var[1], var[0]]
        ...
        # Σ_x = diag(σ²_x, σ²_y) - assumes uncorrelated x,y uncertainty
        self.position_cov = np.array([[self.position_variance[0], 0],
    ...
    def _horizon_planning(self, current_pos):
        """
        Multi-step horizon planning (H > 1) with expected info under pose noise.

        Mathematical formulation (Receding-Horizon with Belief-Aware Acquisition):
        --------------------------------------------------------------------------
        Solve the H-step lookahead optimization:

            ψ* = argmax_{x_{k+1:k+H}}  Σ_{h=1}^{H} U_k^{(h)}(x_{k+h})
                                      - λ * Σ_{h=0}^{H-1} c(x_{k+h}, x_{k+h+1})

        where:
            - U_k^{(h)}(x) = expected info gain at step h, accounting for
                            variance reduction from simulated earlier steps
        ...
        what values we'll measure! We:
            1. Add hypothetical locations to GP (with placeholder values)
            2. Compute variance at next candidate
            3. This gives information gain at that future state

        Combined with pose-aware acquisition:
        -------------------------------------
        At each hypothetical future step, we compute EXPECTED info gain:

            U_k^{(h)}(x) = E_{x̃ ~ N(x, Σ_x)} [Δ^{(h)}(x̃)]

        where Δ^{(h)} uses the variance from the GP with simulated locations.

        Implementation:
        ---------------
        1. Enumerate candidate paths (subsampled for tractability)
        2. For each path, simulate variance reduction step-by-step
        3. At each step, compute expected info gain under pose noise
        ...
        Returns:
            ...
            best_first_info: U_k(x*_{k+1}), expected info of first step
            all_scores: expected info scores for all candidates (visualization)
        ...
        for path_indices in product(range(n_subsample), repeat=self.horizon):
            ...
            # ============================================================
            # Expected info gain with variance-only lookahead
            # ============================================================
            total_info = 0.0
            ...
            for x_t in path:
                ...
                # Compute EXPECTED info gain under pose noise at this future state
                # U^{(h)}(x_t) = E_{x̃ ~ N(x_t, Σ_x)} [Δ^{(h)}(x̃)]
                expected_info = expected_information_gain(
                    x_t, temp_gp, self.noise_var,
                    self.position_cov, self.n_mc_samples
                )
                total_info += expected_info
            ...
            if score > best_score:
                ...
                # Recompute expected info for first step (for logging)
                temp_gp = GPModel(lengthscale=self.lengthscale, noise_var=self.noise_var)
                if train_x is not None:
                    temp_gp.fit(train_x, np.zeros(len(train_x)))
                best_first_info = expected_information_gain(
                    path[0], temp_gp, self.noise_var,
                    self.position_cov, self.n_mc_samples
                )

        # Compute expected info scores for all candidates (for visualization) - VECTORIZED
        all_scores = expected_information_gain_vectorized(
            self.candidates, self.gp, self.noise_var,
            self.position_cov, self.n_mc_samples
        )
