...
# Import pg_viz from same directory (when installed) or parent directory (when in development)
_script_dir = os.path.dirname(os.path.abspath(__file__))
# Try same directory first (installed case)
if not os.path.exists(os.path.join(_script_dir, 'pg_viz.py')):
    # Fall back to parent directory (development case)
...
class MIPlanner(Node):
    def __init__(self):
        ...
        self.out_dir = Path.home() / "workspaces" / "aquatic-mapping" / "src" / "info_gain" / "scripts" / "2"
        ...
        # --- Live viz ---
        self.viz = PGLiveViz(W=self.W, H=self.H, xs=self.xs, ys=self.ys, title_suffix="MI + Distance")
        self.viz.planner_id = 2
        self.viz.show()
        ...
        self.viz.set_levels(
            ...
            mi_title="Score"                # label 3rd panel (MI + distance)
        )
        
        self.get_logger().info(f"MI planner (MI + dist) | field [0,{self.W}]x[0,{self.H}] | max_samples={self.max_samples}")
    ...
    def cb_measurement(self, msg):
        ...
        # ONLY sample if we're at the target
        if not self.at_target:
            return
        
        # Prevent duplicate samples at same location
        ...
        self.get_logger().info(f"✓ Sample #{len(self.y_obs)}: actual pos ({self.pose_xy[0]:.2f},{self.pose_xy[1]:.2f}), T={msg.data:.1f}°C")
    ...
    def plan_next(self):
        ...
        # Krause MI (correlation-aware information gain)
        mi_gains = self.compute_krause_mi_gains(self.grid, n_subsample=150)
        
        # Travel distance penalty (from current position)
        travel_dist = np.linalg.norm(self.grid - self.pose_xy, axis=1)
        
        # Combined objective: information gain - travel cost
        # Higher lambda = prefer nearby points more strongly
        lambda_travel = 0.15
        score = mi_gains - lambda_travel * travel_dist
        ...
        chosen_mi = float(mi_gains[best_idx])
        chosen_travel = float(travel_dist[best_idx])
        chosen_score = float(score[best_idx])
        ...
        self.csv_data.append({
            ...
            'krause_mi_gain': chosen_mi,
            'travel_distance': chosen_travel,
            'total_score': chosen_score,
            'mean_variance': mean_var
        })
        
        self.send_waypoint(target[0], target[1])
        self.get_logger().info(f"→ Next: ({target[0]:.1f},{target[1]:.1f}), score={chosen_score:.3f}, MI={chosen_mi:.3f}, travel={chosen_travel:.2f}m, mean_var={mean_var:.3f}")
        ...
        self.viz.push(mu, std, score, self.X_obs, self.current_target, self.y_obs, self.entropy_history, self.mi_history)
    ...
    def send_waypoint(self, x, y):
        ...
        # Set yaw toward target for correct RViz arrow orientation
        try:
    ...
    def save_csv(self):
        import csv
        csv_path = self.out_dir / "planner_2_metrics.csv"
        with open(csv_path, 'w', newline='') as f:
            writer = csv.DictWriter(f, fieldnames=['sample', 'waypoint_x', 'waypoint_y', 'actual_x', 'actual_y', 'temperature', 'krause_mi_gain', 'travel_distance', 'total_score', 'mean_variance'])
            writer.writeheader()
...
def main():
    ...
    # Spin ROS in a background thread (safe)
    ros_thread = threading.Thread(target=rclpy.spin, args=(node,), daemon=True)
    ros_thread.start()
    
    try:
        # Run Qt GUI in the main thread (required)
        node.viz.run()
